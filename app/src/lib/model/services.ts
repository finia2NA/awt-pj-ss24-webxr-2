...
function getServices() {
  ...
  const contentGuideData = castToArray(dvbi.rawData.ServiceList.ContentGuideSourceList?.ContentGuideSource);

  const contentGuideMap = {};
  for (let contentGuide of contentGuideData) {
    const programInfoEndpoint = contentGuide.ProgramInfoEndpoint;
    const scheduleInfoEndpoint = contentGuide.ScheduleInfoEndpoint;

    contentGuideMap[contentGuide["@_CGSID"]] = {
      name: contentGuide.ProviderName,
      programInfoEndpoint: getUri(programInfoEndpoint),
      scheduleInfoEndpoint: getUri(scheduleInfoEndpoint),
    }
  }
...
function getUri(data): string {
  const uri = getNamespacedValue(data, "URI") ?? getNamespacedValue(data, "MediaUri");
  return typeof uri === "string" ? uri : uri?.["#text"];
}
...
class Service {
  ...
  constructor(rawServiceData, contentGuideMap) {
    ...
    if (rawServiceData.RelatedMaterial) {
      const relatedMaterials = castToArray(rawServiceData.RelatedMaterial);
    
      relatedMaterials.forEach(relatedMaterial => {
        const howRelated = getNamespacedValue(relatedMaterial, "HowRelated");
        const howRelatedHref = howRelated?.["@_href"];
        if (howRelatedHref?.endsWith(":1001.2")) {
          const mediaLocator = getNamespacedValue(relatedMaterial, "MediaLocator");
          this.logoUrl = getUri(mediaLocator);
        }
      });
    }
